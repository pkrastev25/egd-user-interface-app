...
import com.egd.userinterface.iot.controllers.interfaces.IMotorController
...
(
        ...
        @IotGpioPortsRaspberryPiAnnotation input: String,
        @IotGpioPwmRaspberryPiAnnotation output: String,
        dutyCycle: Double,
        frequency: Double
) : IMotorController {
    ...
    init {
        ...
        try {
            mPWMOutput = IotGpioUtil.configurePwm(
                    output,
                    dutyCycle,
                    frequency
            )
        } catch (e: IOException) {
            Log.e(TAG, "IotGpioUtil.configurePwm()", e)
        }
